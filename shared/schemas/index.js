...
export const step2Schema = z.object({
  ...
  ageOfBuilding: z.enum(Object.values(AgeOfBuilding), { required_error: "Please select age of building" }),
  ...
  parking:       z.enum(Object.values(ParkingType), { required_error: "Please select parking availability" }),
  furnishing:    z.enum(Object.values(FurnishingStatus), { required_error: "Please select furnishing type" }),
  pricePerSqft:  z.number().min(0).optional(),
  askPrice:      z.number({ required_error: "Ask price is required" }).min(1, "Ask price must be greater than 0"),
  priceUnit:     z.enum(Object.values(PriceUnit)).default(PriceUnit.LAKHS),
  sbua:          z.number({ required_error: "SBUA is required" }).min(1, "Enter a valid SBUA in sqft"),
});
...
export const step3Schema = z.object({
  ...
  description:     z.string().min(VALIDATION.DESCRIPTION.MIN, `Description must be at least ${VALIDATION.DESCRIPTION.MIN} characters`).max(VALIDATION.DESCRIPTION.MAX),
});

export const inventoryCreateSchema = step1Schema.omit({ photoCount: true }).merge(step2Schema).merge(step3Schema);
